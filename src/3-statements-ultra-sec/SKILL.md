---
name: 3-statements-ultra-sec
description: >
  从零构建机构级三表模型（IS/BS/CF）— 优先通过 SEC filings + notebooklm-py 抽取财务数据，完整公式联动、季度/半年/年频自适应、IFRS/US GAAP/中国准则。
  触发词：三表模型、financial model、3-statement、建模、从零建模、收入预测、SEC、EDGAR、notebooklm。
  ❌ 填写已有模板请用 financial-analysis:3-statements
---

# 3-Statement Model — SEC + NotebookLM Fork (v4.8 · SEC-First Edition)
...
For US SEC filers, this fork assumes your SEC EDGAR email is available in an env file
such as `~/Programs/.env` under `SEC_EDGAR_EMAIL`.
...
**This fork changes the default source hierarchy.**

If the company is a US SEC filer, the default path is:

1. `SEC filings download`
2. `NotebookLM notebook seeded with those SEC filings`
3. `Excel upload` if the user already has structured historicals
4. `Web` only as cross-check / fallback

Only skip SEC + NotebookLM if one of the following is true:

- the company is not an SEC filer
- NotebookLM auth is unavailable
- the user explicitly wants a faster non-SEC workflow
- the environment cannot access EDGAR or NotebookLM

For the SEC-first flow, read [references/sec-notebooklm.md](references/sec-notebooklm.md) before Session A.
Use the bundled scripts when possible instead of rewriting the bootstrap logic:

- `scripts/sec_nlm_bootstrap.py`
- `scripts/nlm_extract_company_pack.py`

**⚠️ Data Source Guide — Read Before Starting**

| Option | Setup | Token Cost | Recommended? |
|--------|-------|-----------|--------------|
| **SEC filings -> NotebookLM** (download with EDGAR, seed notebook automatically) | One-time OAuth auth setup + SEC email env | Very low after setup | ✅ Best path for US filers |
| **NotebookLM notebook** (annual reports / prospectus pre-loaded) | One-time OAuth auth setup | Very low — NLM handles the PDF; Claude only receives answers | ✅ Best path for non-SEC or pre-seeded cases |
...
After login, auth is stored at `~/.notebooklm/storage_state.json`.

For US SEC filers, bootstrap the source stack with:
```bash
