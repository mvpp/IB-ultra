...
### SEC-First override for this fork

如果目标公司是 **US SEC filer**，且环境满足以下条件：

- 已登录 `notebooklm`
- 已安装 `notebooklm-py`
- 可访问 EDGAR
- `SEC_EDGAR_EMAIL` 可从 env 读取

则不要把 Web 或 Excel 当作默认一号源。先执行：

```bash
python scripts/sec_nlm_bootstrap.py --ticker [TICKER]
python scripts/nlm_extract_company_pack.py --notebook-id [NOTEBOOK_ID]
```

并读取 [references/sec-notebooklm.md](sec-notebooklm.md)。

完成后将 `_State` 写成：

```text
DATA_SOURCES: [SEC, NLM, WEB]
DATA_CONFIDENCE: HIGH
SEC_NOTEBOOK_ID: <NOTEBOOK_ID>
```

Web 只作为 cross-check 层运行。

### 用户提问（非 SEC-first 情况或需补充来源）
...
□ A) SEC filings + NotebookLM
      → 如果是美股 / SEC 披露公司，优先选这个

□ B) NotebookLM notebook — 有这家公司的年报/季报/招股书 notebook
      → 请提供 notebook 链接或名称关键词

□ C) Excel 历史数据 — 有整理好的历史财务 Excel（IS/BS/CF 已在里面）
      → 请上传文件
...
```
DATA_SOURCES: [SEC, NLM, EXCEL, WEB]   ← 实际用到的来源列表
DATA_CONFIDENCE: HIGH / MEDIUM / LOW
  HIGH   = SEC+NLM 可用，或 NLM + EXCEL 同时可用（双源交叉验证）
...
### 1A — SEC -> NotebookLM（US SEC filer 默认）
...
1. `10-K` / `10-Q` / earnings-relevant `8-K` 先从 EDGAR 下载
2. 将下载的 filing text 全部送入单独 NotebookLM notebook
3. 使用 NotebookLM 做 0A / 0B 抽取
4. 只有当 filing 中找不到的 forward guidance / KPI，才去 Web cross-check

如果 bootstrap 成功，把 notebook id 写入 `_State`：

```python
write_state_key(wb, "SEC_NOTEBOOK_ID", NOTEBOOK_ID)
write_state_key(wb, "DATA_SOURCES", "[SEC, NLM, WEB]")
write_state_key(wb, "DATA_CONFIDENCE", "HIGH")
```

### 1B — NLM Batch（如用户选了预置 notebook 或 SEC-first 已完成）

安装依赖：
```bash
pip install "notebooklm-py[browser]"   # 若尚未安装
```
